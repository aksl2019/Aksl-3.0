...
{
    public class CustomerGroupHubViewModel : BindableBase, INavigationAware
    {
        #region Members
        ...
        #region Constructors
        public CustomerGroupHubViewModel(IUnityContainer container, IEventAggregator eventAggregator, IDialogViewService dialogViewService, ICustomerService customerService)
        {
        ...
        internal async Task CreateCustomersViewModelAsync()
        {
            ...
            try
            {
                var customerGroupViewModel = _container.Resolve<CustomerGroupViewModel>();
                customerGroupViewModel.PropertyChanged += (sender, e) =>
                {
                    if (sender is CustomerGroupViewModel cgvm)
                    {
                        if ( e.PropertyName==nameof(CustomerGroupViewModel.IsLoading) && !cgvm.IsLoading)
                        {
                            IsLoading = false;
                        }
                    }
                };
                customerGroupViewModel.CreateGroupedCustomersViewModelsAsync().GetAwaiter().GetResult();
                Workspaces.Add(customerGroupViewModel);
                SetActiveWorkspace(customerGroupViewModel);
