            ...
            if (messages.Count() > 0)
            {
                using (await _mutexRead.LockAsync())
                {
                    ...
                    foreach (var msg in messages)
                    {
                        ...
                        {
                            _logger.LogError($"Error while process a order: {ex.Message} in { nameof(WebSocketListener)}.{ nameof(ProcessJsonStringFromClientAsync)}");
                        }
                    }
                    ...
                    //    // await  _dataflowProducerConsumer.ProduceAsync(new List<SaleOrderDto>(saleOrderDtos)).AsTask().ForceAsync();
                    //    // _dataflowProducerConsumer.ProduceAsync(new List<SaleOrderDto>(saleOrderDtos)).AsTask().ForceAsync().GetResult();
                    ...
                    //if (saleOrderByteList.Count() > 0)
                    //{
                    //    await _pipeProducerConsumerEx.ProduceAsync(saleOrderByteList);
                    //}

                    //orderByteList = await WebSocketListener.Instance.DataflowBulkInsertBlockTasksAsync(saleOrderDtos.ToArray());
