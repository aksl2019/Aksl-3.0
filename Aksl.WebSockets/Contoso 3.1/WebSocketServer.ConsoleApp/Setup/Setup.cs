...
using Aksl.WebSockets.Listener.Configure;
using Aksl.WebSockets.Listener.Processor;
...
namespace WebSocketServer.ConsoleApp
{
    public partial class WebSocketListener
    {
        ...
        public static WebSocketListener Instance { get; private set; }
        #endregion

        #region Constructors
        static WebSocketListener()
        {
        ...
        public Task InitializeTask()
        {
            try
            {
                ...
                //WebSocket
                this.Services.Configure<WebSocketTransportOptions>((webSocketTransportOptions) =>
                {
                    this.Configuration.Bind("WebSocketTransport", webSocketTransportOptions);
                });

                this.Services.UseWebSockets((webSocketListenOptions) =>
                {
                    webSocketListenOptions.ContentRootPath = basePath;

                    // socketServerOptions.ApplicationSchedulingMode = SchedulingMode.ThreadPool;

                    this.Configuration.Bind("WebSocket", webSocketListenOptions);

                    var webSocketSection = this.Configuration.GetSection("WebSocket");

                    webSocketListenOptions.ConfigureEndpointDefaults(listenOptions =>
                    {
                       // listenOptions.PipeSettings.MinAllocBufferSize = 2048;
                    });

                    webSocketListenOptions.Configure(webSocketSection)
                                          .Load();

                    //var endPointConfigurationLoader = webSocketListenOptions.Configure();
                ...
                _logger = _loggerFactory.CreateLogger<WebSocketListener>();
                ...
                _pipeProducerConsumer = new PipeProducerConsumer<SaleOrderDto>(async (saleOrderDtos) =>
               {
                ...
                webSocketListener.AddJsonStringHandler(async (buffer, token) =>
                {
                ...
                try
                {
                    ...
                    async Task InsertSaleOrders()
                    {
                        if (_queues.Count > 0)
                        {
                            ...
                            _queues.Clear();
                            await WebSocketListener.Instance.DataflowBulkInsertBlockTasksAsync(saleOrderDtos);
                        }
                    }
                }
