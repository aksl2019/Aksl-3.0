...
namespace Contoso.ConsoleApp
{
    ...
    {
        #region Block Dataflow Bulk Insert Tasks
        public async Task DataflowBulkInsertBlockTasksAsync(int taskCount = 2,  int orderCount = 5000)
        {
            if (!_isInitialize)
            {
            ...
            var insertTasks = from tc in Enumerable.Range(0, taskCount)
                              select Task.Run(() =>
                              {
                                  return DataflowBulkInsertBlockCoreAsync(signals, tc, orderCount);
        ...
        public async Task DataflowBulkInsertBlockCoreAsync(AsyncCountdownEvent signals, int index,  int orderCount)
        {
            #region Block Method
            ...
            {
                logger.LogError($"Error while dataflow bulk insert orders of {nameof(DataflowPipeBulkInsertLoopCoreAsync)}: {ex.Message}");
            }
        }
        #endregion
    }
