...
namespace Contoso.ConsoleApp
{
    public partial class WebApiSender
    {
        ...
        public async Task DataflowBulkInsertRetryTasksAsync(int seqNumber = 1, int frequency = 1000, int taskCount = 2, int count = 1, int orderCount = 100, int maxRetryCount = 2)
        {
            if (!_isInitialize)
            {
            ...
            var logger = _loggerFactory.CreateLogger($"DataflowBulkInsert-{seqNumber}-OrderCount:{totalOrderCount}");
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    var insertTasks = from tc in Enumerable.Range(0, taskCount)
                                      select Task.Run(() =>
                                      {
                                          return DataflowBulkInsertCoreAsync(signals, tc, count, orderCount);
                ...
                logger
                  .LogInformation($"----finish dataflow bulk insert {totalOrderCount} orders,cost time:\"{executionTimeWatcher.Elapsed}\",transport time:{ totalTransportTime },count/time(sec):{Math.Ceiling(totalOrderCount / totalTransportTime.TotalSeconds)},now:\"{DateTime.Now.TimeOfDay}\"----");
            }
            catch (Exception ex)
            {
        ...
        public async Task DataflowBulkInsertCoreAsync(AsyncCountdownEvent signals, int index, int count, int orderCount)
        {
            ...
            try
            {
               ...
               // logger.LogInformation($"----begin dataflow bulk insert { totalOrderCount} orders,now:{DateTime.Now.TimeOfDay}----");
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    transportTimeWatcher.Restart();
                    var dbOrders = await orderDataSource.DataflowBulkInsertSaleOrdersAsync(orders);
                    totalTransportTime += transportTimeWatcher.Elapsed;
                    transportTimeWatcher.Reset();
            ...
            {
                logger.LogError($"Error while dataflow bulk insert orders of {nameof(DataflowBulkInsertCoreAsync)}: {ex.Message}");
            }
        }
        #endregion
    }
