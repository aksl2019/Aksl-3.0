...
namespace Contoso.ConsoleApp
{
    public partial class WebApiSender
    {
        ...
        public async Task PipeBulkInsertRetryTasksAsync(int seqNumber = 1, int frequency = 1000, int taskCount = 2, int count = 1, int orderCount = 100, int maxRetryCount = 2)
        {
            if (!_isInitialize)
            {
            ...
            var logger = _loggerFactory.CreateLogger($"PipeBulkInsert-{seqNumber}-OrderCount:{totalOrderCount}");
            ...
            try
            {
                ...
                logger.LogInformation($"----begin pipe bulk insert {totalOrderCount} orders,now:{DateTime.Now.TimeOfDay}----");
                ...
                while (true)
                {
                    ...
                    var insertTasks = from tc in Enumerable.Range(0, taskCount)
                                      select Task.Run(() =>
                                      {
                                          return PipeBulkInsertCoreAsync(signals, tc, count, orderCount);
                ...
                logger
                    .LogInformation($"----finish pipe bulk insert {totalOrderCount} orders,cost time:\"{executionTimeWatcher.Elapsed}\",transport time:{ totalTransportTime },count/time(sec):{Math.Ceiling(totalOrderCount / totalTransportTime.TotalSeconds)},now:\"{DateTime.Now.TimeOfDay}\"----");
            }
            catch (Exception ex)
            {
        ...
        public async Task PipeBulkInsertCoreAsync(AsyncCountdownEvent signals, int index, int count, int orderCount)
        {
            var logger = _loggerFactory.CreateLogger($"PipeBulkInserter-TaskCount:{index}");
            ...
            try
            {
               ...
               // logger.LogInformation($"----begin dataflow bulk insert { totalOrderCount} orders,now:{DateTime.Now.TimeOfDay}----");
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    transportTimeWatcher.Restart();
                    var dbOrders = await orderDataSource.PipeBulkInsertSaleOrdersAsync(orders);
                    totalTransportTime += transportTimeWatcher.Elapsed;
                    transportTimeWatcher.Reset();
            ...
            {
                logger.LogError($"Error while pipe bulk insert orders of {nameof(PipeBulkInsertCoreAsync)}: {ex.Message}");
            }
        }
        #endregion
    }
